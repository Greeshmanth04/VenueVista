...
//Add hall
adminApp.post("/hall", expressAsyncHandler(async(req, res)=>{
    let hallsCollection = req.app.get("hallsCollection");
    let hall = req.body;
...
//Update hall
adminApp.put("/hall", expressAsyncHandler(async(req, res)=>{
    let hallsCollection = req.app.get("hallsCollection");
    let hall = req.body;
...
//Block hall
adminApp.put("/block-hall/:name", expressAsyncHandler(async(req, res)=>{
    let hallsCollection = req.app.get("hallsCollection");
...
//Delete hall
adminApp.delete("/hall/:name", expressAsyncHandler(async (req, res) => {
...
//Block user 
adminApp.put("/block-user/:email", expressAsyncHandler(async(req, res)=>{
    let usersCollection = req.app.get("usersCollection");
    let email = req.params.email;
    let dbuser = await usersCollection.findOne({email: email});
    if(dbuser == null){
...
//verify booking
adminApp.put("/verify-booking/:bookingID", expressAsyncHandler(async(req, res)=>{
    let bookingsCollection = req.app.get("bookingsCollection");
...
//Block booking
adminApp.put("/block-booking/:bookingID", expressAsyncHandler(async(req, res)=>{
    let bookingsCollection = req.app.get("bookingsCollection");
    let bookingID = Number(req.params.bookingID);
    let dbBooking = await bookingsCollection.findOne({bookingID: bookingID});
    if(dbBooking == null){
...
//Protected
adminApp.get("/protectedLogin", verifyToken, expressAsyncHandler(async(req, res)=>{
