...
function Halls() {
  ...
  useEffect(()=>{
    async function getHalls(){
      let res = await axios.get("http://localhost:4000/admin-api/halls");
  ...
  async function handleHallUpdate(hallData){
    ...
    if(tokenRes.data.message == "token verified successfully"){
      ...
      let res = await axios.put("http://localhost:4000/admin-api/hall", hallData);
      setHalls(prevHalls => prevHalls.map(h => h.id === selectedHall.id ? {...h, ...hallData} : h));
      closePopup();
    }
    else{
      setMsg(tokenRes.data.message);
    }
  }
  ...
  async function handleAddHall(hallData){
    ...
    if(tokenRes.data.message == "token verified successfully"){
      ...
      hallData.blockStatus = false;
      hallData.image = postImage.myFile;
      let res = await axios.post("http://localhost:4000/admin-api/hall", hallData);
  ...
  async function deleteHall(){
    ...
    let tokenRes = await axios.get("http://localhost:4000/admin-api/protectedLogin", config);
    if(tokenRes.data.message == "token verified successfully"){
      let res = await axios.delete(`http://localhost:4000/admin-api/hall/${selectedHall.name}`);
