...
function VerifyUsers() {
  ...
  useEffect(() => {
    async function getUsers() {
      let res = await axios.get(`${BASE_URL}/admin-api/users`);
  ...
  async function verifyUser(userObj){
    ...
    if(tokenRes.data.message == "token verified successfully"){
      let res = await axios.put(`${BASE_URL}/admin-api/verify-user/${userObj.email}`);
  ...
  async function rejectUser(userObj){
    ...
    let tokenRes = await axios.get(`${BASE_URL}/admin-api/protectedLogin`, config);
    if(tokenRes.data.message == "token verified successfully"){
      let res = await axios.put(`${BASE_URL}/admin-api/block-user/${userObj.email}`);
