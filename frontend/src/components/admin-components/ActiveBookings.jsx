...
function ActiveBookings() {
  ...
  async function handleSelection(hallname) {
    ...
    if(tokenRes.data.message == "token verified successfully"){
      let res = await axios.get(`${BASE_URL}/admin-api/hall-bookings/${hallname}`);
  ...
  async function blockBooking(bookingObj){
    ...
    let tokenRes = await axios.get(`${BASE_URL}/admin-api/protectedLogin`, config);
    if(tokenRes.data.message == "token verified successfully"){
      let res = await axios.put(`${BASE_URL}/admin-api/block-booking/${bookingObj.bookingID}`);
                ...
                bookings.map((booking, idx) => (
                  <tr key={idx} className={booking.activeStatus == false ? "row blockedRow" : "row"}>
                    ...
                    <td>{new Date(booking.date) >= today ? 
                          <div className="block" onClick={()=>blockBooking(booking)}>⃠</div> 
                          : <p>Completed</p>
                    }</td>
                  </tr>
                ))
