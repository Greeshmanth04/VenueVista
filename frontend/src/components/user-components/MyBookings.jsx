...
function MyBookings() {
  ...
  useEffect(() => {
    async function getBookings() {
      let res = await axios.get(`http://localhost:4000/user-api/user-bookings/${user.email}`);
  ...
  useEffect(() => {
    async function getHalls() {
      let res = await axios.get(`http://localhost:4000/user-api/halls`);
  ...
  async function cancelBooking(bookingObj){
    ...
    if(tokenRes.data.message == "token verified successfully"){
      let res = await axios.put(`http://localhost:4000/user-api/block-booking/${bookingObj.bookingID}`);
            ...
            {bookings.length > 0 ? (
              bookings.map((booking, idx) => (
                <tr key={idx} className="row">
                  ...
                  <td>
                    {booking.activeStatus == true && halls.find(hall => hall.name == booking.hallname) ? <p className="cancel" onClick={()=>cancelBooking(booking)}>Cancel</p> : <p className="none">---</p>}
                  </td>
                </tr>
              ))
            ) : (
